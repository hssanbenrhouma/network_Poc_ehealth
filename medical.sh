...
function printHelp () {
  echo "Usage: "
  echo "  medical.sh up|down|restart|generate|reset|clean|upgrade|createneworg|startneworg|stopneworg [-c <channel name>] [-f <docker-compose-file>] [-i <imagetag>] [-o <logfile>] [-dev]"
  echo "  medical.sh -h|--help (print this message)"
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"medicalchannel\")"
  ...
  echo
  echo "	medical.sh generate -c medicalchannel"
  echo "	medical.sh up -c medicalchannel -o logs/network.log"
  echo "        medical.sh up -c medicalchannel -i 1.1.0-alpha"
  echo "	medical.sh down -c medicalchannel"
  echo "        medical.sh upgrade -c medicalchannel"
  echo
  echo "Taking all defaults:"
  echo "	medical.sh generate"
  echo "	medical.sh up"
  echo "	medical.sh down"
}
...
function newOrgNetworkUp () {
  checkPrereqs
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config/peerOrganizations/exportingentityorg.medical.com" ]; then
...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.medical.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.medical.com
  docker cp -a orderer.medical.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.medical.com
  docker-compose $COMPOSE_FILES up --no-deps orderer.medical.com

  for PEER in peer0.providerorg.medical.com peer0.pharmacyorg.medical.com peer0.hospitalorg.medical.com peer0.regulatororg.medical.com; do
...
function newOrgNetworkDown () {
  ...
  for PEER in peer0.exportingentityorg.medical.com; do
    # Remove any old containers and images for this peer
    CC_CONTAINERS=$(docker ps -a | grep dev-$PEER | awk '{print $1}')
    if [ -n "$CC_CONTAINERS" ] ; then
      docker rm -f $CC_CONTAINERS
...
function replacePrivateKey () {
  ...
  if [ "$DEV_MODE" = true ] ; then
    CURRENT_DIR=$PWD
    cd crypto-config/peerOrganizations/devorg.medical.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed $OPTS "s/DEVORG_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  else
    # The next steps will replace the template's contents with the
    # actual values of the private key file names for the two CAs.
    CURRENT_DIR=$PWD
    cd crypto-config/peerOrganizations/providerorg.medical.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed $OPTS "s/PROVIDER_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/pharmacyorg.medical.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed $OPTS "s/PHARMACY_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/hospitalorg.medical.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed $OPTS "s/HOSPITAL_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/regulatororg.medical.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
...
function replacePrivateKeyForNewOrg () {
  ...
  # actual values of the private key file names for the two CAs.
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/exportingentityorg.medical.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateCertsForNewOrg (){
  ...
  if [ -d "crypto-config/peerOrganizations/exportingentityorg.medical.com" ]; then
    rm -Rf crypto-config/peerOrganizations/exportingentityorg.medical.com
  fi
  set -x
...
# for the sample network. There are five members - one Orderer Org (``MedicalOrdererOrg``)
# and four Peer Orgs (``ProviderOrg``, ``PharmacyOrg``, ``HospitalOrg`` & ``RegulatorOrg``)
# each managing and maintaining one peer node.
# This file also specifies a consortium - ``MedicalConsortium`` - consisting of our
# four Peer Orgs.  Pay specific attention to the "Profiles" section at the top of
# this file.  You will notice that we have two unique headers. One for the orderer genesis
# block - ``FourOrgsMedicalOrdererGenesis`` - and one for our channel - ``FourOrgsMedicalChannel``.
# These headers are important, as we will pass them in as arguments when we create
# our artifacts.  This file also contains two additional specifications that are worth
# noting.  Firstly, we specify the anchor peers for each Peer Org
# (``peer0.providerorg.medical.com`` & ``peer0.pharmacyorg.medical.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "###########################################################"
  if [ "$DEV_MODE" = true ] ; then
    PROFILE=OneOrgMedicalOrdererGenesis
    CHANNEL_PROFILE=OneOrgMedicalChannel
  else 
    PROFILE=FourOrgsMedicalOrdererGenesis
    CHANNEL_PROFILE=FourOrgsMedicalChannel
  fi
  ...
  if [ "$DEV_MODE" = false ] ; then
    ...
    echo
    echo "#####################################################################"
    echo "#######  Generating anchor peer update for PharmacyOrgMSP  ##########"
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/PharmacyOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg PharmacyOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for PharmacyOrgMSP..."
      exit 1
    fi

    echo
    echo "####################################################################"
    echo "#######  Generating anchor peer update for HospitalOrgMSP  ##########"
    echo "####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/HospitalOrgMSPanchors.tx -channelID $CHANNEL_NAME -asOrg HospitalOrgMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for HospitalOrgMSP..."
      exit 1
    fi
